...
export default class PlayerStateMachine {
    constructor(player) {
        ...
        this.stateData[this.currentState].init(player); // Set the current state data
    }
    //change state
    setState(newState) {
        if (!this.stateData[this.currentState].transitions.includes(newState)) {
            ...
            this.player.currentState = this.currentState; // Update the player's current state
            this.stateData[this.currentState].init(this.player); // Call init of the new state
    ...
    update(deltaTime) {
        ...
        if (currentStateData) {
            currentStateData.update(this.player, deltaTime); // Call update of the current state
    ...
    sprintUpdate(dt) {
        ...
        if (this.player.stamina <= 0) {
            this.setState(PlayerEnums.PlayerStates.IDLE); // Transition to idle if stamina is depleted
            return;
        }
        this.player.pos=this.player.pos.add(this.player.vel.scale(dt)); // Update position based on velocity
