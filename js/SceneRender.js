...
export default class SceneRender {
    ...
    render(rays,player) {
        ...
        rays.forEach((ray,i) => {
            ...
            const img=Textures.textures.types.sprites.player[0];
            if(player.currentFrame&&player.frameOffset){
                ctx.drawImage(img,player.currentFrame[0],player.currentFrame[1],player.currentFrame[2],player.currentFrame[3],screenWidth-player.frameOffset[0],screenHeight-player.frameOffset[1],player.frameOffset[2],player.frameOffset[3]);

                if(player.dualWield){
                    ctx.save();
                    ctx.scale(-1,1);
                    ctx.drawImage(img,player.currentFrame[0],player.currentFrame[1],player.currentFrame[2],player.currentFrame[3],-player.frameOffset[0],screenHeight-player.frameOffset[1],player.frameOffset[2],player.frameOffset[3])
                    ctx.restore();
                }
            }
